...
public class NPCOverviewEvent extends EventNode<NullState> {
  
  private NPC npc;
  private Event<?> returnEvent;
  
  // for saving/loading
  public NPCOverviewEvent() {
    this(null, null);
  }
  ...
  public NPCOverviewEvent(final NPC npc, final Event<?> returnEvent) {
    this.npc = npc;
  ...
  protected void doEvent() {
    ...
    for (final TextWrapper txtWrp : this.npc.getGeneralDescription()) {
      addText(txtWrp);
    }
    
    addOption(Key.OPTION_LEAVE, CommonText.OPTION_BACK, this.returnEvent);
    for (final Option option : this.npc.getOverviewOptions(this.returnEvent)) {
