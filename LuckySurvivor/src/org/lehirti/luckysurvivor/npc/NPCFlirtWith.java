...
public class NPCFlirtWith extends EventNode<NullState> implements Externalizable {
  ...
  public NPCFlirtWith(final NPC npc, final Event<?> returnEvent) {
    this.npc = npc;
  ...
  protected void doEvent() {
    ...
    for (final TextWrapper txtWrp : this.npc.getFlirtWithDescription()) {
      addText(txtWrp);
    }
    
    addOption(Key.OPTION_LEAVE, CommonText.OPTION_BACK, new NPCOverviewEvent(this.npc, this.returnEvent));
    for (final Option option : this.npc.getFlirtWithOptions(this.returnEvent)) {
