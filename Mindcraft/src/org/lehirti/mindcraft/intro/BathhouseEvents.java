...
public class BathhouseEvents implements LocationHook {
  ...
  public Map<Event<?>, Double> getCurrentEvents() {
    ...
    if (StateObject.is(Bool.GOT_BLOWJOB_IN_BATHHOUSE)) {
      events.put(new StandardEvent(Intro.THREE_GIRLS_IN_BATHHOUSE, Intro.THREE_GIRLS_IN_BATHHOUSE, new HomeVillage()),
          Double.valueOf(100.0));
      if (C.DUCKGIRLS.available) {
        events.put(new StandardEvent(Intro.FUTA_DAY_IN_BATHHOUSE, Intro.FUTA_DAY_IN_BATHHOUSE, new HomeVillage()),
            Double.valueOf(100.0));
      }
    } else {
      if (StateObject.is(Bool.YOU_ARE_HORNY)) {
        ...
        events.put(new StandardEvent(Intro.GUYS_ONLY_IN_BATHHOUSE, Intro.GUYS_ONLY_IN_BATHHOUSE, new HomeVillage()),
            Double.valueOf(100.0));
      }
    }
    return events;
